//advanced lighting shader that implements oren-nayar diffuse and cook-torrance specular reflectance
#version 410 
...
void main() 
{
	...
	else
	{
		...
		//Fresnel Term F
		float F = reflectionCoefficient + (1 - reflectionCoefficient) * pow(1 - NdE, 5.0f);
		...
		lightOutput = lightDiffuse.rgb * orenNayar;
		specularOutput = lightSpecular.rgb * specular.rgb * cookTorrance;
	}
}
